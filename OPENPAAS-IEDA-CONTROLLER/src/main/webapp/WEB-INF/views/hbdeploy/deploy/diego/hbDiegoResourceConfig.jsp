...
var resourceLayout = {
        ...
        *********************************************************/
        grid: {
            name: 'diego_resource_config_grid',
            header: '<b>리소스 정보</b>',
            method: 'GET',
                multiSelect: false,
            show: {
...
$(function(){
    $('#diego_resource_config_grid').w2layout(resourceLayout.layout2);
    ...
    $("#deleteBtn").click(function(){
        ...
        var selected = w2ui['diego_resource_config_grid'].getSelection();
        if( selected.length == 0 ){
            w2alert("선택된 정보가 없습니다.", "리소스 정보 삭제");
            return;
        }
        else {
            var record = w2ui['diego_resource_config_grid'].get(selected);
            w2confirm({
                title       : "리소스",
                ...
                yes_callBack: function(event){
                    deleteHbDiegoResourceConfigInfo(record.id, record.resourceConfigName);
                },
                no_callBack    : function(){
                    w2ui['diego_resource_config_grid'].clear();
                    doSearch();
                }
            });
        }
    ...
    if(iaas == ""){
        ...
        $.ajax({
            ...
            error : function( request, status, error ) {
                ...
                w2alert(errorResult.message, "DIEGO 리소스 정보 관리");
                w2utils.unlock($("#layout_layout_panel_main"));
                resetForm();
            }
        });
...
function getStemcellList(directorInfo){
    ...
    if(directorInfo == ""){
        ...
        $.ajax({
            ...
            error : function(request, status, error) {
                var errorResult = JSON.parse(request.responseText);
                w2alert(errorResult.message, "DIEGO 리소스 정보 저장");
                w2utils.unlock($("#layout_layout_panel_main"));
                resetForm();
            }
        });
...
function doSearch() {
    ...
    resetForm();
    w2ui['diego_resource_config_grid'].clear();
    w2ui['diego_resource_config_grid'].load('/deploy/hbDiego/resource/list');
}
/********************************************************
 * 설명 : 초기 버튼 스타일
 * 기능 : doButtonStyle
 ...
 *********************************************************/
function registHbDiegoResourceConfigInfo(){
    ...
    $.ajax({
        type : "PUT",
        url : "/deploy/hbDiego/resource/save",
        contentType : "application/json",
        async : true,
...
/********************************************************
 * 설명 : 리소스 정보 삭제
 * 기능 : deleteHbDiegoResourceConfigInfo
 *********************************************************/
function deleteHbDiegoResourceConfigInfo(id, resourceConfigName){
    ...
    $.ajax({
        type : "DELETE",
        url : "/deploy/hbDiego/resource/delete",
        contentType : "application/json",
        async : true,
        data : JSON.stringify(resourceConfigInfo),
...
function clearMainPage() {
    $().w2destroy('layout2');
    $().w2destroy('diego_resource_config_grid');
}
/********************************************************
 * 설명 : 인증서 리셋
 * 기능 : resetForm
...
<div id="main">
    <div class="page_site"> 이종 DIEGO 설치 > <strong>리소스 정보 관리</strong></div>
    <!-- 사용자 목록-->
    <div class="pdt20">
        <div class="title fl"> 리소스 정보 목록</div>
    </div>
    <div id="diego_resource_config_grid" style="width:100%;  height:700px;"></div>
...
<div id="regPopupDiv" hidden="true" >
    ...
    <div id="regPopupBtnDiv" style="text-align: center; margin-top: 5px;">
        <sec:authorize access="hasAuthority('DEPLOY_HBDIEGO_RESOURCE_ADD')">
        ...
        <sec:authorize access="hasAuthority('DEPLOY_HBDIEGO_RESOURCE_DELETE')">
...
$(function() {
    ...
    $("#settingForm").validate({
        ...
        rules: {
            ...
            registHbDiegoResourceConfigInfo();
        }
    });
});
</script>