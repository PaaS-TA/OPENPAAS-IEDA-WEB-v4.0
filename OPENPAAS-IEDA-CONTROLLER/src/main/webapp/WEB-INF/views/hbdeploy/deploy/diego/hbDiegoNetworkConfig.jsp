        ...
        if( selected.length == 0 ){
            ...
            w2confirm({
                ...
                yes_callBack: function(event){
                    deleteDiegoNetworkConfigInfo(record.recid, record.networkConfigName);
                },
 ...
 *********************************************************/
 function addInternalNetworkInputs(preDiv, form){
    ...
    var nIndex = 2;
    var nDiv = preDiv.split("_")[0] + "_" +nIndex;
    ...
    var html= "<div class='panel' style='margin-top:2%;'>";
        ...
        if(index==2){
            html+=    '<span class="btn btn-info btn-sm addInternal" onclick="addInternalNetworkInputs(\''+nDiv+'\', \''+form+'\');">추가</span>&nbsp&nbsp'; 
        }
        ...
        if( iaas.toLowerCase() == "aws" ){
            html+= "<div id='availabilityZoneDiv"+index+"' class='w2ui-field'>"+"<label style='width:40%;text-align: left;padding-left: 20px;'>" + "가용 영역" + "</label>"; 
 ...
 function delInternalNetwork(preDiv, index){
      ...
      $(form + " "+ div).html("");
      $(preDiv+" .addInternal").css("display", "inline-block");
 }
 ...
 function iaasTypeChangeInput(value, record){
    ...
    if( value.toUpperCase() == "AWS" ){
        ...
        if($("#defaultNetworkInfoDiv_2").html() != ""){
            ...
            if($("#defaultNetworkInfoDiv_3").html() != ""){
                $("#availabilityZoneDiv3").show();
                $("#availabilityZoneDiv3").css("display", "block");
            }
        }
    } else {
        ...
        if($("#defaultNetworkInfoDiv_2").html() != ""){
            ...
            if($("#defaultNetworkInfoDiv_3").html() != ""){
                $("#availabilityZoneDiv3").hide();
                $("#availabilityZoneDiv3").css("display", "none");
            }
        }
    }
}
...
function settingNetworkInfo(){
    ...
    $.ajax({
        type: "GET",
        url: "/deploy/hbDiego/network/list/detail/"+record.networkConfigName,
...
function doSearch() {
    ...
    w2ui['network_GroupGrid'].clear();
    w2ui['network_GroupGrid'].load('/deploy/hbDiego/network/list');
    doButtonStyle(); 
}
...
/********************************************************
 * 설명 : 네트워크 정보 등록
 * 기능 : registDiegoNetworkConfigInfo
 *********************************************************/
function registDiegoNetworkConfigInfo(form){
...
function deleteDiegoNetworkConfigInfo(id, networkConfigName){
    ...
    $.ajax({
        type : "DELETE",
        url : "/deploy/hbDiego/network/delete",
        contentType : "application/json",
        async : true,
...
<div id="main">
    <div class="page_site">이종 DIEGO 설치 > <strong>Network 정보 관리</strong></div>
    <!-- 사용자 목록-->
    <div class="pdt20">
...
<div id="regPopupDiv" hidden="true" >
    ...
    <div id="regPopupBtnDiv" style="text-align: center; margin-top: 5px;">
        <sec:authorize access="hasAuthority('DEPLOY_HBDIEGO_NETWORK_ADD')">
        ...
        <sec:authorize access="hasAuthority('DEPLOY_HBDIEGO_NETWORK_DELETE')">
    ...
    $("#defaultNetworkInfoForm").validate({
        ...
        rules:{
           subnetId_1 : {
            ...
            registDiegoNetworkConfigInfo($(form).attr("id"));
        }
    });
});
