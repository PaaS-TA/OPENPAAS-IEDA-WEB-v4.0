...
public class HbBootstrapDeployAsyncService {
    ...
    *****************************************************************/
    public void deployBootstrap(HbBootStrapDeployDTO dto, Principal principal) {
        ...
        try {
            ...
            if( bootstrapInfo != null ) {
                ...
                bootstrapInfo = new HbBootstrapVO(); 
            }
            ...
            if( deploymentFile.exists() ) {
                ...
                ProcessBuilder builder = new ProcessBuilder("bosh", "create-env", deployFile, 
                        ...
                        "--vars-store="+HYBRID_CREDENTIAL_DIR+ bootstrapInfo.getDefaultConfigVo().getCredentialKeyName() , "--tty");
            ...
            if ( status.equalsIgnoreCase("error") ) {
                ...
                if ( accumulatedLog.contains("Failed deploying") || accumulatedLog.contains("Failed")) {
                    ...
                    // 타겟 테스트
                    DirectorRestHelper.sendTaskOutput(principal.getName(), messagingTemplate, MESSAGE_ENDPOINT, "started", Arrays.asList("","BOOTSTRAP 디렉터 정보 : https://" + bootstrapInfo.getNetworkConfigVo().getPublicStaticIp() + ":25555"));
     ...
     * @return : BootstrapVO
    *****************************************************************/
    public HbBootstrapVO saveDeployStatus(HbBootstrapVO bootstrapVo) {
        if ( bootstrapVo == null ) {
            return null;
        }
